namespace FantasyHelper.Services.FPL
{
    public class FPLEmailService : IEmailService
    {
        ...
        public FPLEmailService(ILogger<FPLEmailService> logger, IOptions<EmailOptions> emailOptions, IOptions<SmtpOptions> smtpOptions, PlayersFactory playersFactory)
        {
            _logger = logger;
        ...
        public async Task SendDailyMail()
        {
            try
            {
                ...
                var result = await EmailHelpers.SendEmail(new()
                {
                    ...
                    Body = EmailHelpers.ConstructEmailBaseContent("Daily FPL Update", ConstructDailyEmailContent(risingPlayers, fallingPlayers)),
                });
            }
            catch (Exception ex)
            {
